...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    const shopOrder = order.shopOrders.find((o) => o.shop == shopId.toString());
    if (!shopOrder) {
...
export const getDeliveryBoyAssignment = async (req, res) => {
  try {
    ...
    // brodcastedTo is array, so use $in
    ...
    return res
      .status(500)
      .json({ message: `get Assignment delivery error: ${error.message}` });
  }
};
...
export const acceptOrder = async (req, res) => {
  try {
    ...
    // 3. Agar user already dusre order pe assigned hai to prevent karo
...
export const getCurrentOrder = async (req, res) => {
  try {
    ...
    if (!assignment.shopOrderId) {
      return res
        .status(400)
        .json({ message: "shopOrderId missing in assignment" });
    }
...
export const getTodayDeliveries = async (req, res) => {
  try {
    ...
    orders.forEach((order) => {
      order.shopOrders.forEach((shopOrder) => {
        if (
          ...
          shopOrder.status == "delivered" &&
          shopOrder.deliveryAt && // 👈 corrected
    ...
    todaysDeliveries.forEach((shopOrder) => {
      const hour = new Date(shopOrder.deliveryAt).getHours(); // 👈 corrected
    ...
    return res
      .status(500)
      .json({ message: `today delivery error: ${error.message}` });
  }
};
